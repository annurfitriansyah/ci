...
<div class="content-page">
    ...
    <div class="content">
        <div class="container">
            ...
            <div class="row">
                <div class="col-sm-12">
                    <h1><b>Data Pengeluaran</b></h1>
                    <div>
                        <button class="btn btn-custom btn-default waves-effect waves-light" onclick="input_pengeluaran()"><i
                                class="fa fa-plus"></i> Tambah Pengeluaran </button>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        table = $('#data').DataTable({
            ...
            "ajax": {
                "url": "<?=site_url('pengeluaran/ajax_list')?>",
                "type": "POST"
            },
    ...
    function input_pengeluaran() {
        save_method = 'add';
        $('#form-pengeluaran')[0].reset(); // reset form on modals
        $('.form-group').removeClass('has-error'); // clear error class
        $('.help-block').empty(); // clear error string

        $('.modal-title').text('Form Input Pengeluaran');
        $('#modal_pengeluaran').modal('show'); // show bootstrap modal 
    }
    ...
    function edit_pengeluaran(id_pengeluaran) {
        ...
        $.ajax({
            url: "<?=site_url('pengeluaran/ajax_pengeluaran')?>/" + id_pengeluaran,
            type: "GET",
            dataType: "JSON",
            success: function (data) {
                ...
                $('.modal-title').text('Ubah Data Pengeluaran ' + data
                .sumber); // Set title to Bootstrap modal title

                $('#upload').show(); // show photo preview modal
                $('#upload div').html('<img src="' + base_url + 'uploads/pengeluaran/' + data.gambar +
    ...
    function save() {
        ...
        if (save_method == 'add') {
            url = "<?=site_url('pengeluaran/ajax_add_pengeluaran')?>";
        } else {
            url = "<?=site_url('pengeluaran/ajax_update_pengeluaran')?>";
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                $('#btnSave').text('Simpan Pengeluaran'); //change button text
            ...
            error: function (jqXHR, textStatus, errorThrown) {
                ...
                $('#btnSave').text('Coba Simpan Lagi Pengeluaran'); //change button text
                $('#btnSave').attr('disabled', false); //set button enable 
    ...
    function delete_pengeluaran(id_pengeluaran, sumber) {
        var url;
        url = "<?=site_url('pengeluaran/ajax_delete_pengeluaran')?>";
        swal({
                title: "Are you sure?",
            ...
            function () {
                swal("Deleted!", "Data Pengeluaran has been deleted.", "success");
                reload_table();
                $.ajax({
                    url: url,
                    type: 'POST',
                    ...
                    error: function (jqXHR, textStatus, errorThrown) {
                        ...
                        $('#btnSave').text('Hapus Data Pengeluaran'); //change button text
                        $('#btnSave').attr('disabled', false); //set button enable 
...
<div id="modal_pengeluaran" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="modal_pengeluaranLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="" id="form-pengeluaran">
                    <input type="hidden" name="id_pengeluaran">

                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                            <label for="field-3" class="control-label">Sumber Pengeluaran</label>
                                <select class="selectpicker m-b-0" data-style="btn-white" name="sumber">
                                    <option value="0">Pilih Sumber Pengeluaran</option>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                            <label for="field-3" class="control-label">Jenis Pengeluaran</label>
                                <select class="selectpicker m-b-0" data-style="btn-white" name="jenis" id="jenis">
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <input type="text" name="jenis2" required="" class="form-control" id="jenis_baru" placeholder="Lain-Lain" style="display:none" >
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="form-group">
                                <label for="field-3" class="control-label">Keterangan Pengeluaran</label>
                                <textarea class="form-control autogrow" id="field-7" name="keterangan" required=""
                                    placeholder="Keterangan Pengeluaran (Misal : Buat apa?, dll)"
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" name="jumlah" required="" class="form-control" id="field-3"
                                    onkeypress="return hanyaAngka(event)" placeholder="Jumlah" required>
                                <span class="help-block"></span>
                            </div>
                        </div>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                ...
                                <input type="file" class="filestyle" name="gambar" data-buttontext="Pilih File"
                                    data-placeholder="Upload Gambar" data-buttonname="btn-custom btn-info">
                            </div>
                        </div>
                    </div>
    ...
    <script>
        ...
        $("#jenis").change(function () {
            ...
            if ($("#jenis option:selected").val() == 'Lain-lain') {
                $('#jenis_baru').css('display', 'block');
            } else {
                $('#jenis_baru').prop('display', 'none');
            }
        });
