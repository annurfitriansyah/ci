...
class List_masjid extends CI_Controller{
    function __construct(){
        parent::__construct();
        $this->load->model('list_masjid_model');
    ...
    function index(){
        ...
        $this->theme_admin->load('list_masjid_view',$data);
    }
 
    public function ajax_list(){
		$this->load->helper('url');

		$list = $this->list_masjid_model->get_datatables();
		$data = array();
        $no = 1;
		...
		$output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->list_masjid_model->count_all(),
                        "recordsFiltered" => $this->list_masjid_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function ajax_list2(){
		...
		$list = $this->list_masjid_model->get_datatables2();
		$data = array();
        $no = 1;
		foreach ($list as $item) {
            $row = array();
		...
		$output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->list_masjid_model->count_all2(),
                        "recordsFiltered" => $this->list_masjid_model->count_filtered2(),
                        "data" => $data,
                );
        //output to json format
        echo json_encode($output);
    ...
    public function ajax_update_masjid()
    {
        ...
        $insert = $this->list_masjid_model->edit_masjid(array('id_masjid' => $this->input->post('id_masjid')), $data);
        echo json_encode(array("status" => TRUE));
    }
