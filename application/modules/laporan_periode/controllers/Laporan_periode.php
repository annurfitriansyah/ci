...
class Laporan_periode extends CI_Controller{
    function __construct(){
        parent::__construct();
        $this->load->model('laporan_periode_model');
    ...
    function index(){
        ...
        if ($level == 0) {
            $this->theme_admin->load('laporan_periode_view', $data);
        } else {
            $this->theme_pengelola->load('laporan_periode_view', $data);
        }
    ...
    public function ajax_list(){
        ...
        $list = $this->laporan_periode_model->get_datatables();
        $data = array();
        $no = 1;
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->laporan_periode_model->count_all(),
                        "recordsFiltered" => $this->laporan_periode_model->count_filtered(),
                        "data" => $data,
                );
    ...
    {   $data['user'] = $this->db->get_where('admin_masjid', ['email' => $this->session->userdata('email')])->row_array();
        ...
        $this->Laporan_periode_model->save_data($data);
    ...
    {   
        ...
        $kegiatan= $this->laporan_periode_model->kegiatan_list($id_kegiatan);
        ...
        $insert = $this->laporan_periode_model->edit_kegiatan(array('id_kegiatan' => $this->input->post('id_kegiatan')),
        $data);
        echo json_encode(array("status" => TRUE));
    }
    ...
    public function ajax_delete_kegiatan()
    {
        ...
        $this->laporan_periode_model->delete_kegiatan($id);
        echo json_encode(array("status" => TRUE));
    ...
    {
        $data = $this->laporan_periode_model->get_kegiatan_by_id($id);
        echo json_encode($data);
    }

    function tampil_kegiatan(){
        $data = $this->laporan_periode_model->kegiatan_list();
